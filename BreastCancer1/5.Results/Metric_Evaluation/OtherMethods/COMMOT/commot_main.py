...
bc1_root = base_dir.parent.parent.parent.parent
combo_path = (bc1_root / "2.LR_Screening"
...
result = []
for lr in lr_keys:
    obsp_key = f'commot-{db_name}-{lr[0]}-{lr[1]}'
    if obsp_key not in adata_dis500.obsp:
        print(f'Skipping {lr}: key {obsp_key!r} not found in obsp (pair not computed by spatial_communication)')
        continue
    ...
    try:
        ...
        print(f'Skipping {lr}: cluster_communication raised KeyError {e}')
        continue
