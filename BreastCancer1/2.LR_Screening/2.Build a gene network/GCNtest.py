...
data_path = BASE_DIR.parent / "1.Data preprocessing" / "t_wavelet-all-L.mat"
mat = sio.loadmat(data_path)
if 't_data' not in mat:
    raise KeyError(f"'t_data' not found in {data_path}")
if 'point' not in mat:
    raise KeyError(f"'point' not found in {data_path} (needed for prewavelet-L.mat / c_index0.m)")
# Same objects as MATLAB getpre.m: Rec_time, name (= point); saved after GCN with maprho
rec_time_for_save = np.asarray(mat['t_data'], dtype=np.float64)
point_for_save = mat['point']
...
df_avg.to_excel(BASE_DIR / 'L-averaged_final_adj-.xlsx', index=False, header=False)

# Equivalent to getpre.m: prewavelet-L.mat for c_index0.m (Rec_time, maprho, name)
sio.savemat(
    str(BASE_DIR / 'prewavelet-L.mat'),
    {
        'Rec_time': rec_time_for_save,
        'maprho': df_avg.to_numpy(dtype=np.float64),
        'name': point_for_save,
    },
)

print(f'Processing completed for cluster k=')
#print(f'Results saved to: R-averaged_final_adj-A2.xlsx')