...
load(fullfile(script_dir, '..', '..', '2.Build a gene network', 'prewavelet-L.mat'));
...
%%%%%%%%%%%%%%%%%
% Save figures
savefig(figureHandel, 'entropy-L.fig');
...
%% Error visualization
save error-L.mat delta_record
%// delta_plot = delta_record;
% // delta_plot(delta_plot > 10) = 0;
error = mean(delta_record, 'all');

[X, Y] = meshgrid(1:88, 1:length(name));
Stem3D(X, Y, delta_plot')

% savefig(gcf, 'error-L.fig');
% save init_indext_wavelet-L.mat H0 t0 H