#!/usr/bin/env python3
"""
GPU-accelerated LR scoring (A3 version)

默认输入文件：
- de_coords_matched.csv
- ligand_expr_by_cell_A3.csv
- receptor_expr_by_cell_A3.csv

默认输出文件：
- LR_scores_all_pairs_V0_meta_gpu_A3.csv（列含 mean_score_recv = 分数列和 / 全部细胞数，按其降序；不再写 mean_score）
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Compute LR scores with GPU (V0 weights, A3)")
    p.add_argument("--input-dir", default=None, help="Input directory. Defaults to this script directory.")
    p.add_argument("--output-dir", default=None, help="Output directory. Defaults to this script directory.")
    p.add_argument("--ligand-file", default="ligand_expr_by_cell_filtered-A3.csv", help="Ligand expression CSV filename")
    p.add_argument("--receptor-file", default="receptor_expr_by_cell_filtered-A3.csv", help="Receptor expression CSV filename")
    p.add_argument("--coords-file", default="de_coords_matched.csv", help="Coordinates CSV filename")
    p.add_argument("--output-file", default="LR_scores_all_pairs_V0_meta_gpu_A3.csv", help="Output CSV filename")
    p.add_argument("--alpha", type=float, default=0.5, help="Ligand competition factor alpha_const")
    p.add_argument("--w-near", type=float, default=0.000467, help="Weight for d < d1")
    p.add_argument("--d1", type=float, default=1300.0, help="Distance threshold 1")
    p.add_argument("--d2", type=float, default=4900.0, help="Distance threshold 2")
    p.add_argument("--kappa", type=float, default=0.000385, help="Decay parameter for mid distances")
    p.add_argument("--lambda_", type=float, default=0.00212, help="Decay parameter for far distances")
...
def load_inputs(input_dir: Path, ligand_file: str, receptor_file: str, coords_file: str):
    lig = pd.read_csv(input_dir / ligand_file)
    rec = pd.read_csv(input_dir / receptor_file)
    coords = pd.read_csv(input_dir.parent / "1.Preprocessing" / coords_file)
...
def main():
    ...
    print(f"Ligand file: {args.ligand_file}")
    print(f"Receptor file: {args.receptor_file}")
    print(f"Coords file: {args.coords_file}")
    print("Reading inputs...")

    lig, rec, coords, cell_cols = load_inputs(input_dir, args.ligand_file, args.receptor_file, args.coords_file)
