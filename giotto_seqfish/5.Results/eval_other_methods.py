...
def build_key_ud(cell1: str, cell2: str) -> str:
    return "||".join(sorted([canonical_lr_node(cell1), canonical_lr_node(cell2)]))
...
def load_known_pairs(path: str | Path, nrows: int = 342) -> pd.DataFrame:
    ...
    known["key_ud"] = known.apply(lambda r: build_key_ud(r.Cell1, r.Cell2), axis=1)
...
def normalize_pred(df: pd.DataFrame, col1: str, col2: str, score_col: str) -> pd.DataFrame:
    ...
    pred["key_ud"] = pred.apply(lambda r: build_key_ud(r.Cell1, r.Cell2), axis=1)
    return pred
...
def _scores_on_frame(frame: pd.DataFrame, pred: pd.DataFrame, key_col: str = "key_dir") -> np.ndarray:
    """Return a score vector aligned to *frame* (0 for keys absent in pred)."""
    pred_dedup = (
        pred[[key_col, "score"]]
        .groupby(key_col, as_index=False)["score"]
        .max()
    )
    merged = frame[[key_col]].merge(pred_dedup, on=key_col, how="left")
    assert len(merged) == len(frame), (
        f"_scores_on_frame: merge expanded ({len(merged)} vs {len(frame)}). "
        "Check for duplicate keys in pred after dedup."
    )
...
def main():
    ...
    print(f"[Config] loaded {len(known)} validated positives (nrows={known_nrows}, skip header row)")
    ...
    pred_map["Spagat-ccc"] = load_pred_scores(
        root.parent / "4.Model_Training" / "pred_scores.csv", topk=topk
    )
