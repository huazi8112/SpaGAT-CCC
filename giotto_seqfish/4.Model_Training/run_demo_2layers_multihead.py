import os
os.environ["PYTHONHASHSEED"] = "2020"
...
lig_df = pd.read_csv(scoring_dir / "ligand_expr_by_cell_filtered-A3.csv", index_col=0)
rec_df = pd.read_csv(scoring_dir / "receptor_expr_by_cell_filtered-A3.csv", index_col=0)
...
adata = train_STAGATE(
    ...
    use_corr_loss=True,
    sigma_expr='auto',
    lr_score_rds=scoring_dir / "LR_scores_all_pairs_V0_meta_gpu_A3.csv",
    ligand_section="s1",
    receptor_section="s2",
...
if att_list is None or len(att_list) == 0:
    ...
    results_pred_path = root.parent / "5.Results" / "pred_scores.csv"
    pred_all[['Cell1', 'Cell2', 'att']].to_csv(results_pred_path, index=False)

    print(f"已保存全部跨层边打分到: {pred_path}")
    print(f"已同步保存到: {results_pred_path}")
