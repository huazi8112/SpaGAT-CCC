    ...
    if use_corr_loss:
        ...
        if cross_df.shape[0] == 0:
            ...
            if sigma_expr is None or (isinstance(sigma_expr, (int, float)) and sigma_expr <= 0):
                ...
                if str(sigma_expr).lower() == 'auto':
                    _median_d = float(np.median(np.sqrt(expr_dist_sq)))
                    _sigma = _median_d if _median_d > 0 else 1.0
                    print(f"[sigma_expr=auto] median ||diff||={_median_d:.2f}, auto sigma={_sigma:.2f}")
                else:
                    _sigma = float(sigma_expr)
                expr_sim = np.exp(-expr_dist_sq / (2.0 * _sigma * _sigma)).astype(np.float32)
            ...
            prior = (prior / (np.float32(np.max(prior)) + _eps)).astype(np.float32)

            prior_map = {(r, c): v for r, c, v in zip(rows, cols, prior)}
            aligned = []
            for r, c in zip(G_tf[0][:, 0], G_tf[0][:, 1]):
