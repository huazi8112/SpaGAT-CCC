...
def _draw_strip(
    ...
    *,
    ...
    if title:
        ax.set_title(title, fontsize=FS.TITLE + 10, fontweight="bold", pad=14,
...
def main():
    ...
    _draw_strip(
        ...
        title=(
            f"A. 空间先验得分（乳腺癌2）— "
            f"N={len(combo_bc2)}个配体-受体对，{len(order_bc2)}个细胞类型对"
        ),
        x_rotation=0.0,
        x_ha="center",
        show_ylabel=True,
    ...
    panel_titles = [
        (
            f"B. 空间先验得分（seqFISH）— 按中位数排序的前 {len(quarters[0])} 个细胞类型对 "
            f"[共 N={len(combo_sf)} 个配体-受体对]"
        ),
    ] + [
        (
            f"B（续）. seqFISH — 第 {i+1}/{n_sf_rows} 张 "
            f"[排名 {i*q_step+1}–{min((i+1)*q_step, len(order_sf))}]"
        )
