...
def draw_dotplot(ax, df_all, top_n=TOP_N_LR):
    ...
    ax.set_title(
        "B. 高置信度空间配体-受体对",
        fontsize=FS.TITLE + 10, fontweight="bold", pad=60,
        loc="left", x=0.085,
        fontfamily="Microsoft YaHei",
    )
...
def plot_network_chart(ax, df):
    ...
    ax.set_title('C. 关键分子枢纽与信号网络（前60）',
                 fontsize=FS.TITLE + 10, pad=18, fontweight='bold',
                 fontfamily="Microsoft YaHei")
    ax.axis('off')
...
def place_strongest_flow_between_a_c_seqfish(fig, ax_a, ax_c, flow):
    ...
    txt = f"最强通讯流：{best['Sender']} \u2192 {best['Receiver']}"
