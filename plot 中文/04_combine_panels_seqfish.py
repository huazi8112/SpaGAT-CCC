"""
图类 04 — 六格合并图（GRN / 扰动 / 阈值等），数据集 seqFISH。
SeqFish (A3) 矢量版 — 将 6 个 panel 合并为真正的矢量 EPS。
...
输出：与本脚本同目录 — 04_combined_panels_seqfish.eps / .png
"""
...
LR_DIR      = BASE / 'giotto_seqfish' / '2.LR_Screening'
GENE_NETWORK_DIR = LR_DIR / '2.Build a gene network'
...
# ── A3 文件名常量 ─────────────────────────────────────────────────────────────
_PREWAVELET_L = 'prewavelet-L-A3.mat'
_PREWAVELET_R = 'prewavelet-R-A3.mat'
_ADJ_L        = 'L-averaged_final_adj-A3.xlsx'
_INIT_IDX_L   = 'init_indext_wavelet-L-A3.mat'
_NEW_NAME_L   = 'new_name-L0.21-A3.mat'
_NEW_NAME_R   = 'new_name-R0.002-A3.mat'
_ADDU_L       = 'adduwavelet-100-(all)-L-merged-A3.mat'
_ADDU_R       = 'adduwavelet-100-(all)-R-merged-A3.mat'
...
CT_COLORS = {
    "astrocytes":      "#4e79a7", "endothelial":   "#f28e2b",
    "microglia":       "#e15759", "mural":         "#76b7b2",
    "Olig":            "#59a14f", "OPC":           "#edc948",
    "Adarb2-iNeuron":  "#b07aa1", "Lhx6-iNeuron":  "#ff9da7",
    "L2-L3-eNeuron":   "#9c755f", "L4-eNeuron":    "#bab0ac",
    "L5-eNeuron":      "#d37295", "L6-eNeuron":    "#a0cbe8",
}
...
def _load_sensitive_names(mat_path):
    ...
    key = 'new_name' if 'new_name' in m else 'name'
    arr_sq = m[key].squeeze()
    return [str(arr_sq[i][0] if hasattr(arr_sq[i], '__len__') else arr_sq[i])
            for i in range(len(arr_sq))]
...
# ============================================================================
# Panel A — GRN hub-burst 网络（A3）
# ============================================================================
def _draw_A(subfig, top_k: int = 10):
    print("[A] Building GRN network (A3) …")
    prewavelet    = sio.loadmat(str(GENE_NETWORK_DIR / _PREWAVELET_L))
    new_name_data = sio.loadmat(str(SUBNET_DIR  / _NEW_NAME_L))
    adj_df = pd.read_excel(str(GENE_NETWORK_DIR / _ADJ_L), header=None)
    init_idx = sio.loadmat(str(ENTROPY_DIR / _INIT_IDX_L))
    entropy_all = np.nanmean(np.array(init_idx['H'], dtype=float), axis=1)

    all_names = [str(prewavelet['name'][i, 0][0])
                 for i in range(prewavelet['name'].shape[0])]
    key_nm = 'new_name' if 'new_name' in new_name_data else 'name'
    _nm = new_name_data[key_nm].squeeze()
    sens_set = set(
        str(_nm[i][0] if hasattr(_nm[i], '__len__') else _nm[i])
    ...
    # ── 裁剪游离节点：超出各自 hub R_clip 半径的节点拉回到边界 ──────────────
    R_clip = 0.85
    for v in list(G_vis.nodes()):
        if v in hub_set:
            continue
        hp = pin_pos[node_hub[v]]
        delta = pos[v] - hp
    ...
    # ── 用 hub + 一跳邻居的坐标范围定轴，避免游离节点撑大坐标轴 ─────────────
    core_nodes = set(hubs)
    for h in hubs:
        core_nodes.update(G_vis.neighbors(h))
    # 保底：若一跳邻居极少，把所有有边的节点也纳入
    if len(core_nodes) < max(5, len(hubs) * 2):
        core_nodes.update(v for v in G_vis.nodes() if G_vis.degree(v) > 0)
    xs = [pos[v][0] for v in core_nodes]; ys = [pos[v][1] for v in core_nodes]

    pad = 0.12
    x_lo, x_hi = min(xs) - pad, max(xs) + pad
    y_lo, y_hi = min(ys) - pad, max(ys) + pad
    tr = 18/15; w, h = x_hi - x_lo, y_hi - y_lo
    if w / h < tr:
...
# ============================================================================
def _draw_B(subfig):
    print("[B] Plotting mean prediction error (A3) …")
    d  = sio.loadmat(str(ENTROPY_DIR / _ERROR_L))
    pw = sio.loadmat(str(GENE_NETWORK_DIR / _PREWAVELET_L))
    ...
    ax.set_title('所有基因的平均预测误差 — L 配体层（A3）',
                 fontsize=34, fontweight='bold', pad=12,
...
# ============================================================================
# Panel C — 蝴蝶图（A3，使用专用中间坐标轴）
# ============================================================================
def _draw_C(subfig):
    print("[C] Plotting butterfly chart (A3) …")
    pre  = sio.loadmat(str(GENE_NETWORK_DIR / _PREWAVELET_L))
    addu = sio.loadmat(str(DISTURB_DIR / _ADDU_L))
    ...
    CT_ABBREV = {
        "astrocytes": "Ast", "endothelial": "End", "microglia": "Mic",
        "mural": "Mur", "Olig": "Oli", "OPC": "OPC",
        "Adarb2-iNeuron": "Adb", "Lhx6-iNeuron": "Lhx",
        "L2-L3-eNeuron": "L23", "L4-eNeuron": "L4",
        "L5-eNeuron": "L5",  "L6-eNeuron": "L6",
    }
    ...
    ax_c = subfig.add_axes([0.39, 0.10, 0.20, 0.76])
    ...
    x_max_l = float(left_bottom.max()) * 1.1 or 3.0
    ax_l.set_xlim(x_max_l, 0)
    ...
    x_max_r = float(right_bottom.max()) * 1.1 or 2.5
    ax_r.set_xlim(0, x_max_r)
...
# ============================================================================
# Panel D — 阈值敏感性曲线（A3，L=0.025，R=0.067）
# ============================================================================
def _draw_D(subfig):
    print("[D] Plotting threshold sensitivity (A3) …")

    def load_names(mat_path):
        mat = sio.loadmat(str(mat_path))
        key = 'new_name' if 'new_name' in mat else 'name'
        arr_sq = mat[key].squeeze()
        return [str(arr_sq[i][0] if hasattr(arr_sq[i], '__len__') else arr_sq[i])
                for i in range(len(arr_sq))]

    def threshold_scan(addu_file, prewavelet_file, layer_tag):
        hp_path = DISTURB_DIR / addu_file
        pre_path = GENE_NETWORK_DIR / prewavelet_file
        if hp_path.is_file() and pre_path.is_file():
            hp = sio.loadmat(str(hp_path))
            pre = sio.loadmat(str(pre_path))
            H = np.array(hp["H_point_value"])
            maprho = np.array(pre["maprho"])
            thresholds = np.round(np.arange(0.005, 0.25 + 1e-9, 0.003), 4)
            counts = []
            for t in thresholds:
        ...
        csv_path = SUBNET_DIR / f"threshold_scan-{layer_tag}-A3.csv"
        if csv_path.is_file():
            df = pd.read_csv(csv_path)
            df = df.assign(thresh=df["thresh"].astype(float))
            df = df.drop_duplicates(subset=["thresh"], keep="last").sort_values("thresh")
            sel = (df["thresh"].values >= 0.005) & (df["thresh"].values <= 0.25)
            return df["thresh"].values[sel], df["nonzero_filtered"].values[sel]
        raise FileNotFoundError(
            f"Panel D: need {hp_path} and {pre_path}, or fallback {csv_path}"
        )

    names_L = load_names(SUBNET_DIR / _NEW_NAME_L)
    names_R = load_names(SUBNET_DIR / _NEW_NAME_R)
    thresh_L, counts_L = threshold_scan(_ADDU_L, _PREWAVELET_L, "L")
    thresh_R, counts_R = threshold_scan(_ADDU_R, _PREWAVELET_R, "R")

    subfig.set_facecolor('white')
    ax = subfig.add_subplot(1, 1, 1)
    ax.plot(thresh_L, counts_L, "o-", color="#4e79a7", linewidth=2, markersize=3,
            label=f"L-layer / Ligand (n={len(names_L)} @ {THRESHOLD_L})")
    ax.plot(thresh_R, counts_R, "s-", color="#e15759", linewidth=2, markersize=3,
            label=f"R-layer / Receptor (n={len(names_R)} @ {THRESHOLD_R})")
    ...
    for thresh, color, layer, thr_arr, cnt_arr in [
        (THRESHOLD_L, "#4e79a7", "L", thresh_L, counts_L),
        (THRESHOLD_R, "#e15759", "R", thresh_R, counts_R),
    ]:
    ...
    ax.set_title("阈值敏感性分析 — SeqFish（A3）",
                 fontsize=34, fontweight="bold", pad=12,
                 fontfamily="Microsoft YaHei")
...
# ============================================================================
# Panel E — L/R 敏感基因数量按细胞类型（A3）
# ============================================================================
def _draw_E(subfig):
    print("[E] Plotting gene count bars (A3) …")
    names_L = _load_sensitive_names(SUBNET_DIR / _NEW_NAME_L)
    names_R = _load_sensitive_names(SUBNET_DIR / _NEW_NAME_R)
    cnt_L = _count_by_ct(names_L); cnt_R = _count_by_ct(names_R)

    all_cts = sorted(set(list(cnt_L) + list(cnt_R)))
    L_vals  = [cnt_L.get(c, 0) for c in all_cts]
    R_vals  = [cnt_R.get(c, 0) for c in all_cts]
    colors  = [CT_COLORS.get(c, '#aaaaaa') for c in all_cts]

    subfig.set_facecolor('white')
    ax = subfig.add_subplot(1, 1, 1)
    x = np.arange(len(all_cts)); w = 0.38
    ...
    ax.set_xticks(x); ax.set_xticklabels(all_cts, fontsize=15, rotation=22, ha='right')
    ax.set_ylabel('Number of sensitive genes', fontsize=20)
    ax.set_title(f'各细胞类型敏感基因数量 — A3'
                 f'（L：{len(names_L)}，R：{len(names_R)}）',
                 fontsize=32, fontweight='bold', pad=12,
                 fontfamily='Microsoft YaHei')
    ax.tick_params(axis='y', labelsize=16)
    ax.spines['top'].set_visible(False); ax.spines['right'].set_visible(False)
    ax.grid(axis='y', linestyle='--', alpha=0.3)
    ymax = max(max(L_vals), max(R_vals)) if max(L_vals + R_vals) > 0 else 1
...
# ============================================================================
# Panel F — L/R 敏感基因平均表达量（A3）
# ============================================================================
def _draw_F(subfig):
    print("[F] Plotting expression curves (A3) …")
    names_L_all, Rec_L = _load_prewavelet(GENE_NETWORK_DIR / _PREWAVELET_L)
    names_R_all, Rec_R = _load_prewavelet(GENE_NETWORK_DIR / _PREWAVELET_R)
    sens_L = set(_load_sensitive_names(SUBNET_DIR / _NEW_NAME_L))
    sens_R = set(_load_sensitive_names(SUBNET_DIR / _NEW_NAME_R))
    ...
    t_L = np.arange(1, expr_L.shape[1] + 1)
    t_R = np.arange(1, expr_R.shape[1] + 1)
    ...
    for ax, t, expr, mean, std, n, color, title in [
        (ax_L, t_L, expr_L, mean_L, std_L, len(idx_L), '#4e79a7', 'L 配体层'),
        (ax_R, t_R, expr_R, mean_R, std_R, len(idx_R), '#e15759', 'R 受体层'),
    ]:
        ...
        ax.set_title(title, fontsize=30, fontweight='bold', pad=10,
                     fontfamily='Microsoft YaHei')
        ax.set_xlabel('Time window', fontsize=18)
...
# ============================================================================
def run():
    print("\n=== Generating fully-vector combined figure (SeqFish / A3) ===")
    ...
    out_eps = PLOT_DIR / f'{FIG_TYPE_ID}_combined_panels_seqfish.eps'
    out_png = PLOT_DIR / f'{FIG_TYPE_ID}_combined_panels_seqfish.png'
