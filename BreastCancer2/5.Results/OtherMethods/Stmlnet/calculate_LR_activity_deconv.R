...
# 
# Input:
#   - ligand_expr_by_cell_filtered.csv (deconvolved)
#   - receptor_expr_by_cell_filtered.csv (deconvolved)
#   - combo_only.csv (L-R pairs)
...
# Set paths
bc2_dir <- "D:/GitHub Code/SpaGAT-CCC/BreastCancer2"
script_dir <- file.path(bc2_dir, "5.Results/OtherMethods/Stmlnet")
...
# Filter to available pairs (only for score computation)
lr_pairs <- combo_df[
  combo_df$ligand %in% rownames(exprMat) &
  combo_df$receptor %in% rownames(exprMat),
]

dropped_pairs <- combo_df[
  !(combo_df$ligand %in% rownames(exprMat) & combo_df$receptor %in% rownames(exprMat)),
]

cat("  L-R pairs to score:", nrow(lr_pairs), "/", nrow(combo_df), "\n")
cat("  Filter rule: ligand and receptor must both exist in expression matrix rownames(exprMat)\n")
cat("  Filtered out pairs:", nrow(dropped_pairs), "\n")
...
# Restore full combo list and fill missing pairs with 0 scores
result_df <- combo_df %>%
  select(ligand, receptor, combo) %>%
  left_join(result_df, by = c("ligand", "receptor", "combo")) %>%
  mutate(
    mean_score = ifelse(is.na(mean_score), 0, mean_score),
    median_score = ifelse(is.na(median_score), 0, median_score),
    max_score = ifelse(is.na(max_score), 0, max_score),
    sum_score = ifelse(is.na(sum_score), 0, sum_score),
    sd_score = ifelse(is.na(sd_score), 0, sd_score),
    ligand_mean_expr = ifelse(is.na(ligand_mean_expr), 0, ligand_mean_expr),
    receptor_mean_expr = ifelse(is.na(receptor_mean_expr), 0, receptor_mean_expr)
  )
...
cat("Filtered out during scoring (then zero-filled in final output):", nrow(dropped_pairs), "\n")
