...
def load_known_pairs(path: str, nrows: int = 766) -> pd.DataFrame:
...
def _load_pred_map(methods_root: Path, spagat_path: Path, topk: int) -> dict[str, pd.DataFrame]:
    pred_map: dict[str, pd.DataFrame] = {}

    stmlnet_path = methods_root / "Stmlnet" / "result_deconv" / "LR_activity_scores.csv"
    if stmlnet_path.exists():
        stmlnet_df = pd.read_csv(stmlnet_path)
        pred_map["Stmlnet"] = normalize_pred(stmlnet_df, "ligand", "receptor", "mean_score")
    else:
        print(f"[Stmlnet] 跳过：未找到结果文件 {stmlnet_path}")

    cellcha_path = methods_root / "CellChatV2" / "result" / "result_combo_only.csv"
    if cellcha_path.exists():
        cellcha_df = pd.read_csv(cellcha_path)
        pred_map["CellChatV2"] = normalize_pred_with_celltype(
            cellcha_df, "Sender", "Receiver", "Ligand", "Receptor", "LRscore"
        )
    else:
        print(f"[CellChatV2] 跳过：未找到结果文件 {cellcha_path}")
    ...
    if commot_path is not None:
        newconm_df = pd.read_csv(commot_path)
        pred_map["COMMOT"] = normalize_pred_with_celltype(
            newconm_df, "Sender", "Receiver", "Ligand", "Receptor", "LRscore"
        )
    else:
        print(f"[COMMOT] 跳过：未找到结果文件 {commot_candidates[0]}")

    cytosignal_path = methods_root / "CytoSignal" / "result" / "CytoSignal_result.csv"
    if cytosignal_path.exists():
        cytosignal_df = pd.read_csv(cytosignal_path)
        pred_map["CytoSignal"] = normalize_pred_with_celltype(
            cytosignal_df, "Sender", "Receiver", "Ligand", "Receptor", "LRscore"
        )
    else:
        print(f"[CytoSignal] 跳过：未找到结果文件 {cytosignal_path}")

    if spagat_path.exists():
        pred_map["Spagat-ccc"] = load_pred_scores(spagat_path, topk=topk)
    else:
...
def main():
    ...
    known = load_known_pairs(combo_path, nrows=765)
