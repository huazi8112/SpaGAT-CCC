...
def _draw_D(subfig):
    ...
    ax.set_title("Threshold sensitivity — SeqFish (A3)",
                 fontsize=24, fontweight="bold", pad=12)
...
def _draw_E(subfig):
    ...
    ax.set_title(f'Sensitive gene counts by cell type — A3  '
...
def _draw_F(subfig):
    ...
    for ax, t, expr, mean, std, n, color, title in [
        (ax_L, t_L, expr_L, mean_L, std_L, len(idx_L), '#4e79a7', 'L-layer'),
        (ax_R, t_R, expr_R, mean_R, std_R, len(idx_R), '#e15759', 'R-layer'),
    ]:
