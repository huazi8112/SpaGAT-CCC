...
def place_strongest_flow_between_a_c(fig, ax_a, ax_c, flow):
    ...
    txt = f"Strongest flow: {best['SenderCT']} \u2192 {best['ReceiverCT']}"
...
def draw_dotplot(ax, pred_all, top_n=30):
    ...
    ax.set_title(
        ...
        fontsize=FS.TITLE, fontweight="bold", pad=36, loc="left", x=0.08, y=1.04,
    )
    ...
    cbar.set_label(
        "Attention score (5–95%ile range)",
...
def draw_edge_bundling(ax, top60):
    ...
    ax.set_aspect("equal")
    ax.axis("off")
    ax.set_title("C. Key Molecular Hubs (Top 60, centrality top 15 labeled)",
    ...
    leg = ax.legend(handles=handles, loc="lower center",
                    ...
                    fontsize=FS.LEGEND, title="Cell Type",
...
def main():
    ...
    ax_a.set_title(
        "A. Cell-Cell Communication Global Strength (Top 100)",
        fontsize=FS.TITLE, fontweight="bold", pad=14,
    )
