"""
图类 04 — 六格合并图（GRN / 扰动 / 阈值等），数据集 BC2。
BreastCancer2 (A2) 矢量版 — 将 6 个 panel 合并为真正的矢量 EPS。
...
输出：与本脚本同目录 — 04_combined_panels_bc2.eps / .png
"""
...
LR_DIR      = BASE / 'BreastCancer2' / '2.LR_Screening'
# prewavelet-L/R-A2.mat 由 2.Build a gene network/GCNtest.py 写出（与 c_index0.m、threshold_scan.py 一致）
...
# ── A2 文件名常量 ─────────────────────────────────────────────────────────────
_PREWAVELET_L = 'prewavelet-L-A2.mat'
_PREWAVELET_R = 'prewavelet-R-A2.mat'
_ADJ_L        = 'L-averaged_final_adj-A2.xlsx'
_INIT_IDX_L   = 'init_indext_wavelet-L-A2.mat'
_NEW_NAME_L   = 'new_name-L0.45-A2.mat'
_NEW_NAME_R   = 'new_name-R0.01-A2.mat'
_ADDU_L       = 'adduwavelet-100-(all)-L-merged-A2.mat'
...
# ============================================================================
# Panel A — GRN hub-burst 网络（A2）
# ============================================================================
def _draw_A(subfig, top_k: int = 10):
    print("[A] Building GRN network (A2) …")
    prewavelet    = sio.loadmat(str(NETWORK_DIR / _PREWAVELET_L))
    sens_set = set(_load_sensitive_names(SUBNET_DIR / _NEW_NAME_L))
    adj_df = pd.read_excel(
        str(LR_DIR / '2.Build a gene network' / _ADJ_L), header=None)
    ...
    # 按基因名微调特定 hub 的钉子坐标（调整前 spring_layout 生效）
    _hub_pin_offset = {
        'LGALS9': np.array([-0.28, 0.0]),
        'MMP9':   np.array([-0.28, 0.0]),
    }
    for v in hubs:
        lbl = G.nodes[v]['label']
        if lbl in _hub_pin_offset:
            pin_pos[v] = pin_pos[v] + _hub_pin_offset[lbl]
    ...
    # 裁剪游离节点：spring_layout 会用斥力把无边节点推到远处，
    # 撑大坐标轴范围，导致真正的网络内容缩成一个点。
    # 将所有非 hub 节点限制在距其所属 hub 最多 max_clip 半径内。
    max_clip = 1.1
    for v in list(pos.keys()):
        if v in hub_set: continue
        hp = pin_pos[node_hub[v]]
        delta = np.asarray(pos[v]) - hp
        dist = float(np.linalg.norm(delta))
        if dist > max_clip:
            pos[v] = hp + delta / dist * max_clip
    ...
    # 用 hub + 每个 hub 的一跳邻居（G_vis 中）的位置确定视口，
    # 避免少数游离远端节点把坐标轴撑得过大。
    vis_nodes = set(hubs)
    for h in hubs:
        vis_nodes.update(G_vis.neighbors(h))
    xs = [pos[v][0] for v in vis_nodes if v in pos]
    ys = [pos[v][1] for v in vis_nodes if v in pos]
    pad = 0.18
...
# ============================================================================
# Panel B — 各时间窗口预测误差均值折线（A2）
# ============================================================================
def _draw_B(subfig):
    print("[B] Plotting mean prediction error (A2) …")
    d  = sio.loadmat(str(ENTROPY_DIR / _ERROR_L))
    pw = sio.loadmat(str(NETWORK_DIR / _PREWAVELET_L))
    ...
    ax.set_title('Mean prediction error across all genes  —  L ligands (A2)',
...
# ============================================================================
# Panel C — 蝴蝶图（A2，使用专用中间坐标轴）
# ============================================================================
def _draw_C(subfig):
    print("[C] Plotting butterfly chart (A2) …")
    pre  = sio.loadmat(str(NETWORK_DIR / _PREWAVELET_L))
...
def _draw_D(subfig):
    ...
    def threshold_scan(addu_file, prewavelet_file):
        hp  = sio.loadmat(str(DISTURB_DIR / addu_file))
    ...
    thresh_L, counts_L = threshold_scan(_ADDU_L, _PREWAVELET_L)
    thresh_R, counts_R = threshold_scan(_ADDU_R, _PREWAVELET_R)
    ...
    ax.set_title("Threshold sensitivity — BreastCancer2 (A2)",
                 fontsize=24, fontweight="bold", pad=12)
...
# ============================================================================
# Panel E — L/R 敏感基因数量按细胞类型（A2）
# ============================================================================
def _draw_E(subfig):
    print("[E] Plotting gene count bars (A2) …")
    ...
    ax.set_title(f'Sensitive gene counts by cell type — A2  '
...
# ============================================================================
# Panel F — L/R 敏感基因平均表达量（A2）
# ============================================================================
def _draw_F(subfig):
    print("[F] Plotting expression curves (A2) …")
    names_L_all, Rec_L = _load_prewavelet(NETWORK_DIR / _PREWAVELET_L)
    names_R_all, Rec_R = _load_prewavelet(NETWORK_DIR / _PREWAVELET_R)
...
# ============================================================================
def run():
    print("\n=== Generating fully-vector combined figure (BreastCancer2 / A2) ===")
    ...
    out_eps = PLOT_DIR / f'{FIG_TYPE_ID}_combined_panels_bc2.eps'
    out_png = PLOT_DIR / f'{FIG_TYPE_ID}_combined_panels_bc2.png'
